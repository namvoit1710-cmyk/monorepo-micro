...
export function isBaseColorTheme(theme: ThemeDefinition): boolean {
...
export function buildThemeCssText(cssVars: ThemeCssVars): string {
    ...
    const lightDecl = toDeclarations(cssVars.light);
    const darkDecl = toDeclarations(cssVars.dark);

    const parts: string[] = [];
    if (lightDecl) parts.push(`:root {\n${lightDecl}\n}`);
    if (darkDecl) parts.push(`.dark {\n${darkDecl}\n}`);
